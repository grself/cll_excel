...
%*****************************************
% Second pass corrections made and grammar checked 220623.
...
Figure \ref{03:fig01} shows the completed workbook that will be demonstrated in this chapter. Notice the techniques used in \textit{Column O} and \textit{Column R} that highlight the results of the calculations, and there is more information on this worksheet than in the original data file.
...
\begin{enumbox}
	\begin{enumerate}
		...
		\item Start typing \fmtTyping{=MAX} (See Figure \ref{03:fig02}). Note the explanation that pops up on the offered list of functions. Either keep typing \fmtTyping{(} (an open parenthesis) or double click \fmtButton{MAX} from the pop-up list.
		...
		\item Select \fmtLoc{B5:B24}. The calculation in each cell will update to: \fmtTyping{=MAX(B5:B24)}. Tap \fmtKeystroke{Enter} to complete the formula.
		\item Now, use \textit{AutoFill} to copy \fmtLoc{B25} to \fmtLoc{C25:N25}. Note that the cell references change as the calculation is copied from one column to the next. The calculation in \fmtLoc{B25} reads: \fmtTyping{=MAX(B5:B24)} and the one in \fmtLoc{N25} reads \fmtTyping{=MAX(N5:N24)}. These cell references are called \textit{relative} references.
	\end{enumerate}
\end{enumbox}

By default, Excel's calculations change their cell references relative to the row or column when copied. That makes sense. \textit{Column N} should not display an answer that uses the values from \textit{Column K}.

To see all the calculations that were just created, press and hold \fmtKeystroke{Ctrl}, then tap \fmtKeystroke{$ ` $} (that is the back-tic near the \fmtKeystroke{one} on the keyboard, see Figure \ref{03:fig03}). Press and hold \fmtKeystroke{Ctrl}, then tap \fmtKeystroke{$ ` $} to display calculations as values, the default view for cells.
...
If necessary, press and hold \fmtKeystroke{Ctrl}, then tap \fmtKeystroke{$ ` $} to return the spreadsheet to the normal view (the results of the formulas in \textit{Row }$ 25 $ should be displayed, not the formulas themselves).
...
\begin{enumbox}
	\begin{enumerate}
		...
		\item In the \fmtButton{Quick Analysis Tool}, select \fmtButton{Totals $ \Rightarrow $ COLUMN SUM} (see Figure \ref{03:fig05}). (Note: there are two SUM buttons, the second will sum columns, as indicated by the tan-colored column indicator on the button.) The \fmtButton{COLUMN SUM} button places a \fmtTyping{=SUM()} calculation in all cells in \fmtLoc{Column O}.
	\end{enumerate}
\end{enumbox}
...
\textit{Column P} requires a Percentage calculation. Before creating a calculation for this, it might be handy to know what is needed. The \textit{Smart Lookup} tool is a handy way to get more information if the computer is connected to the Internet.

\begin{enumbox}
	\begin{enumerate}
		\item Select cell \fmtLoc{P4}.
		\item Click \fmtButton{Review $ \Rightarrow $ Insights $ \Rightarrow $ Smart Lookup} (see Figure \ref{03:fig06}) to read more about Percentage calculations. If this is the first time the \textit{Smart Lookup} tool has been used, a privacy statement may pop up. Tap the \fmtButton{Got it} button.
	\end{enumerate}
\end{enumbox}
...
\begin{enumbox}
	\begin{enumerate}
		...
		\item Tap \fmtTyping{=} then click \fmtLoc{O5}. 
		\item Enter \fmtTyping{/}
		\item Click \fmtLoc{O25}. The calculation should look like \fmtTyping{=O5/O25}.
		\item Tap \fmtKeystroke{Enter}. The cell should display $ 0.95641026 $. So far, so good. DeShea Andrews is doing well in this class, with a percentage grade of almost 96\%---an ``A''!)
		\item Next, use \textit{AutoFill} to copy the calculation from \fmtLoc{P5} to \fmtLoc{P6:P24} to calculate the other students' grades. Unfortunately, this error message is displayed: \textit{\#DIV/$ 0 $}. This message means an attempt has been made to divide a number by $ 0 $ (zero), which is impossible. The formula in \fmtLoc{P9} reads $ =O9/O29 $. The first cell reference is correct --- it points to Moesha Gashi's total points for the class, but the second reference is wrong since it points to an empty cell, \fmtLoc{O29}.
	\end{enumerate}
\end{enumbox}
...
\begin{enumbox}
	\begin{enumerate}
		...
		\item Tap \fmtKeystroke{F4} to make the \fmtLoc{O25} reference absolute, so it will not change when the cell is copied (see Figure \ref{03:fig08}). It is also easy to type a \$ before the \fmtLoc{O} and another before the \fmtLoc{25} for devices like tablets and laptops that may not have function keys.
		\item Tap \fmtKeystroke{Enter}.
		\item The formula in \fmtLoc{P5} now looks like \fmtTyping{=O5/\$O\$25}.
		\item Click in \fmtLoc{P5} and use \textit{AutoFill} to copy that cell to \fmtLoc{P6:P24}. Now the formula has the correct values for all students.
	\end{enumerate}
\end{enumbox}
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Tap \fmtKeystroke{F4} or type a dollar sign (\$) in front of the cell reference column letter and row number.
...
\begin{center}
	\begin{tkwbox}{Key Take-Aways}
		...
		\begin{itemize}
			...
			\item Use an absolute reference to prevent Excel from changing the cell references in a formula or function when pasted to a new cell location. Do this by placing a dollar sign (\$) in front of the column letter and row number of a cell reference or using the \fmtKeystroke{F4} function key.
...
\textit{Column Q} should indicate whether a student is passing or failing the class for the grade book. Students who score $ 70\% $ or better are considered passing, while scores less than $ 70\% $ are failing.
...
\begin{enumbox}
	\begin{enumerate}
		\item Click in the box for \fmtButton{Logical Test}. Enter \fmtTyping{$ P5<0.7 $} to test whether a student's score is less than $ 0.7 $ .
		\item Click in the box for \fmtButton{Value\_if\_true}. If the student's score is less than $ 0.7 $, they fail the class. In this box, type \fmtTyping{Fail}. Note: Excel automatically encloses the word \textit{Fail} in quote marks.
		\item Click in the box for \fmtButton{Value\_if\_false}. If the student's score is \textit{NOT} less than $ 0.7 $, they are passing the class. In this box, type \fmtTyping{Pass}. Note: Excel automatically encloses the word \textit{Pass} in quote marks.
...
\begin{enumbox}
	\begin{enumerate}
		\item Once the function's arguments are entered and checked, tap \fmtButton{OK}. 
		...
		\item Use \textit{AutoFill} to copy \fmtLoc{Q5} to \fmtLoc{Q6:Q24}.
	\end{enumerate}
\end{enumbox}
...
\begin{enumbox}
	\begin{enumerate}
		...
		\begin{itemize}
			\item \textbf{Lookup\_value}. Specify the percentage score in cell \fmtLoc{P5} for the first student.
			\item \textbf{Table\_array}. $ A28 $:$ B32 $ contains the value to be returned by the function. Note that this range does \textit{NOT} include the label in \textit{Row} $ 27 $, just the actual data. It is essential that the cell references for the Table\_array are absolute: \fmtLoc{\$A\$28:\$B\$32}. When this function is copied to other cells, the cell references should not change.
		...
		\item Observe Excel's helpful definitions while filling in the \fmtButton{VLOOKUP} dialog box.
		\item Tap \fmtButton{OK} when the dialog box is complete.
		\item The calculation in the formula bar is: \fmtTyping{=VLOOKUP(P5,\$A\$28:\$B\$32,2)}
		\item Use \textit{AutoFill} to copy \fmtLoc{R5} to \fmtLoc{R6:R24}.
	\end{enumerate}
\end{enumbox}
...
\begin{enumbox}
	\begin{enumerate}
		\item Click in \fmtLoc{A2}. Notice that \fmtLoc{A2} extends from \fmtLoc{Column A} to \fmtLoc{Column R}. Previously, the \fmtButton{Merge \& Center} tool was used on this cell to make it match the width of the title in \fmtLoc{Row 1}.
		\item Click \fmtButton{Formulas $ \Rightarrow $ Function Library $ \Rightarrow $ Date \& Time $ \Rightarrow $ NOW}. 
		\item Click \fmtButton{OK}.
		\item The result in the formula bar is: \fmtTyping{=NOW()} and the result in \fmtLoc{A2} depends on the current date and time. The \fmtButton{NOW} function is convenient; it takes no arguments and is volatile! That is not as alarming as it may seem. A volatile function does not need any information to do its job, and the results will change frequently. 
		\item Wait at least one minute, then click \fmtLoc{A2} and tap \fmtKeystroke{F9} to update the time.
	\end{enumerate}
\end{enumbox}
...
\begin{enumbox}
	\begin{enumerate}
		\item Click in \fmtLoc{A2}. Tap \fmtKeystroke{Delete} to remove the \fmtButton{NOW} function.
...
\begin{center}
	\begin{shtcutbox}{Keyboard Shortcuts}
		...
		\begin{itemize}
			...
			\item Press and hold \fmtKeystroke{CTRL}, then tap \fmtKeystroke{;} (semicolon) to insert the current date
			\item Press and hold \fmtKeystroke{CTRL}, then tap \fmtKeystroke{:} (colon) to insert the current time.
...
\begin{enumbox}
	\begin{enumerate}
		\item Select \fmtLoc{O5:O24}.
		\item At the bottom of the selection, click on the \fmtButton{Quick Analysis Tool}; a pop-up with an icon that looks like a spreadsheet with colored lines.
		\item Click \fmtButton{Formatting $ \Rightarrow $ Data Bars} (see Figure \ref{03:fig18}).
	\end{enumerate}
\end{enumbox}
	
Excel places blue bars indicating the value of each cell to make it easier to see how well each student did in the class.
...
It is even more important to highlight the students who are failing in the class, so that will be done in two places, the \textit{Percentages} and \textit{Letter Grade} columns. As a first effort, any \textit{F} letter grades should be formatted with a light red fill color and dark red text.

\begin{enumbox}
	\begin{enumerate}
		\item Select \fmtLoc{R5:R24}.
...
\begin{enumbox}
	\begin{enumerate}
		...
		\item Fill out the \textit{Less Than} dialog box so that cells that are less than $ 0.6 $ (that is 60\%) will have conditional formatting. Instead of using the default red text on a light red fill, tap the down arrow at the end of the \textit{with} box and select \fmtButton{Custom Format}.
		...
		\item Tap \fmtButton{OK}. Then tap \fmtButton{OK} again.
	\end{enumerate}
\end{enumbox}
...
Conditional Formatting is valuable since it reflects the current data, and it changes whenever the data changes. As a test, delete DeShea's final exam score. (Tap $ N5 $, then \fmtKeystroke{Delete}.) Suddenly, DeShae is failing the course, and the Conditional Formatting reflects that. Press and hold \fmtKeystroke{CTRL}, then tap \fmtKeystroke{Z} (Undo). The test score reappears, and the Conditional formatting reflects that as well.
...
\begin{enumbox}
	\begin{enumerate}
		...
		\item Since there is no need to highlight the students passing the class, click the second rule in the \textit{Rules Manager} (``Cell Value = 'Pass''') and tap the \fmtButton{Delete Rule} button.
	\end{enumerate}
\end{enumbox}
...
In a previous exercise (the \textit{IF} function), it was decided that students failed if they got a percentage score of less than $ 70\% $, so the \textit{Conditional Formatting} rule in the \textit{Percentage} column needs to be changed to match that value.
...
\begin{enumbox}
	\begin{enumerate}
		\item Select \fmtLoc{A1:R25}, which is the only part of the worksheet to be printed.
...
\begin{enumbox}
	\begin{enumerate}
		...
		\item Tap \fmtKeystroke{Enter} to return the \textit{Page Setup} dialog box to its expanded form. Notice that the \textit{Rows to repeat at top} argument is now defined as $ \$1 $:$ \$3 $.
		\item Click \fmtButton{OK}.
	\end{enumerate}
\end{enumbox}

The worksheet does not change in Normal view, so return to \textit{Print Preview}. Notice pages break in the middle of a single park's information, which will be corrected next.

\begin{enumbox}
	\begin{enumerate}
		\item Click \fmtButton{File $ \Rightarrow $ Print}. Notice that the first three rows are repeated each page's top.
		\item Exit \textit{Print Preview} by clicking the arrow button at the top left corner of the preview.
	\end{enumerate}
\end{enumbox}
...
\begin{enumbox}
	\begin{enumerate}
		...
		\item Click \fmtButton{File $ \Rightarrow $ Print}. Check that the date and file name are in the header and that the page numbers in the footer are correct.
		\item Exit \textit{Print Preview} by clicking the arrow button at the top left corner of the preview.
...
\begin{enumbox}
	...
	\end{itemize}
		...
		\item Starting in \fmtLoc{C2}, use \textit{AutoFill} to fill the months \textit{February} through \textit{December} in cells \fmtLoc{D2:N2}. Adjust the widths of \fmtLoc{Column C} to \fmtLoc{Column N} to $ 10 $.
		...
		\begin{enumerate}
			\item Copy \fmtLoc{C3:C4} and paste it to \fmtLoc{D3:N4}.
			\item Copy \fmtLoc{C8:C9} and paste it to \fmtLoc{D8:N9}.
			\item Copy \fmtLoc{C25} and paste it to \fmtLoc{D25:N25}.
			\item Copy \fmtLoc{C27} and paste it to \fmtLoc{D27:N27}.
			\item Copy \fmtLoc{C41} and paste it to \fmtLoc{D41:N41}.
		\end{enumerate}	
		...
		\item Click \fmtButton{Formulas $ \Rightarrow $ Function Library $ \Rightarrow $ AutoSum}. Ensure that Excel selects \fmtLoc{C3:C5} for the \textit{AutoSum} function, then tap \fmtKeystroke{Enter} to calculate the \textit{Total Income} for January.
		\item Use \textit{AutoFill} to copy \fmtLoc{C6} to \fmtLoc{D6:O6}.
	
		\item Select \fmtLoc{C16}.
		\item Click \fmtButton{Formulas $ \Rightarrow $ Function Library $ \Rightarrow $ AutoSum}. Ensure that Excel selects \fmtLoc{C8:C15} for the \textit{AutoSum} function, then tap \fmtKeystroke{Enter} to calculate the \textit{Total Home Expenses} for January.
		\item Use \textit{AutoFill} to copy \fmtLoc{C16} to \fmtLoc{D16:O16}.
		
		\item Select \fmtLoc{C23}.
		\item Click \fmtButton{Formulas $ \Rightarrow $ Function Library $ \Rightarrow $ AutoSum}. Ensure that Excel selects \fmtLoc{C18:C22} for the \textit{AutoSum} function, then tap \fmtKeystroke{Enter} to calculate the \textit{Total Daily Living Expenses} for January.
		\item Use \textit{AutoFill} to copy \fmtLoc{C23} to \fmtLoc{D23:O23}.
		
		\item Select \fmtLoc{C31}.
		\item Click \fmtButton{Formulas $ \Rightarrow $ Function Library $ \Rightarrow $ AutoSum}. Ensure that Excel selects \fmtLoc{C25:C30} for the \textit{AutoSum} function, then tap \fmtKeystroke{Enter} to calculate the \textit{Total Transportation Expenses} for January.
		\item Use \textit{AutoFill} to copy \fmtLoc{C31} to \fmtLoc{D31:O31}.
	
		\item Select \fmtLoc{C37}.
		\item Click \fmtButton{Formulas $ \Rightarrow $ Function Library $ \Rightarrow $ AutoSum}. Ensure that Excel selects \fmtLoc{C33:C36} for the \textit{AutoSum} function, then tap \fmtKeystroke{Enter} to calculate the \textit{Total Entertainment Expenses} for January.
		\item Use \textit{AutoFill} to copy \fmtLoc{C37} to \fmtLoc{D37:O37}.
	
		\item Select \fmtLoc{C45}.
		\item Click \fmtButton{Formulas $ \Rightarrow $ Function Library $ \Rightarrow $ AutoSum}. Ensure that Excel selects \fmtLoc{C39:C44} for the \textit{AutoSum} function, then tap \fmtKeystroke{Enter} to calculate the \textit{Total Personal Expenses} for January.
		\item Use \textit{AutoFill} to copy \fmtLoc{C45} to \fmtLoc{D45:O45}.
		...
		\item Enter an \fmtButton{IF} statement that displays the word \textit{No} if the amount in \fmtLoc{C49} is less than or equal to zero and \textit{Maybe} if the amount is greater than zero. Remember that the words ``No'' and ``Maybe'' must be enclosed in quotes. 
		\item Copy \fmtLoc{C50} to \fmtLoc{D50:N50}.
	
		\item Check to see if the \fmtButton{IF} statement worked correctly in \fmtLoc{Row 50}. If the cells say \textit{No} when the data bar above is red and \textit{Maybe} when the data bar is blue, then the \fmtButton{IF} statement is correct.
...
\begin{enumbox}
	\begin{enumerate}
		...
		\item For the \textit{Shipping Information}, create formulas using cell references to display the corresponding information from the \textit{Billing Information} section. For example, the \textit{Customer} cell will display the customer's name found in \fmtLoc{C11}.
		\item In \fmtLoc{B19:E22}, enter the following item orders:
		...
		\item In cell \fmtLoc{F19}, enter an \fmtButton{IF} function that tests whether the order quantity in cell \fmtLoc{D19} is greater than $ 0 $ (zero). If it is, return the value of the Qty (in \fmtLoc{D19}) multiplied by the Unit Price (in \fmtLoc{E19}); otherwise, return no text by entering ``''.
		...
		\item In cell \fmtLoc{F26}, calculate the sum of all the Item Total cells.
