...
\begin{enumerate}
	\item Make sure ``My table has headers'' is checked. Click \fmtPopupButton{OK}.
...
\begin{enumerate}
	...
	\item Click on the Sales sheet. In I4 , enter a \fmtTyping{Vlookup} function that will find the Product Price for the Product in E4 in the table in the Product Table sheet and return it to I4. In the \fmtTyping{Vlookup} function, fill in the required parameters using Figure \ref{05:fig32} below. Copy the \fmtTyping{Vlookup} function down column I.
\end{enumerate}
...
\begin{enumerate}[resume]
	\item In J4 , enter a \fmtTyping{Vlookup} function that will find the Product Cost for the Product in E4 in the table in the Product Table sheet and return it to J4. This \fmtTyping{Vlookup} function will be the same as the \fmtTyping{Vlookup} function in I4 EXCEPT the COL\_INDEX\_NUM will be $ 3 $ instead of $ 4 $. Copy the function down column J.
