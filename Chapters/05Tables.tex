...
Following these rules will help insure that the sorts, filters, totals, and subtotals applied to the table with return the desired results. With these rules in mind, begin working on the \fmtWorksheetName{Portland ME} worksheet in the National Weather workbook. Notice that the data is in adjacent columns and rows. The upper-left corner of the table is in \fmtCellLocation{A5} and the titles are above the column headings in Row $ 5 $. Since the set-up of the data looks good, it is time to turn the data range into an Excel table.
...
\begin{enumerate}
	...
	\item Click the \fmtPopupButton{Table} button in the \fmtRibbonTab{Insert} tab of the Ribbon.
\end{enumerate}
...
\begin{enumerate}
	\item Make sure \textit{My table has headers} is checked. Click \fmtPopupButton{OK}.
...
\begin{enumerate}
	...
	\item In the \fmtRibbonTab{Table Tools Design} tab, in the \fmtRibbonGroup{Table Styles} group, click the \fmtPopupButton{More} down-arrow button.
\end{enumerate}
...
\begin{enumerate}[resume]
	...
	\item In the \fmtRibbonGroup{Table Style Options} group in the Ribbon, click \fmtPopupButton{Banded Rows}.
\end{enumerate}
...
\begin{table}[H]
	...
	{\small
		%\fontsize{8}{10} \selectfont %Replace small for special font size
		\begin{longtable}{L{0.50in}L{0.50in}L{0.50in}L{0.50in}L{0.50in}} %Left-aligned, Max width: 4.25in
...
\begin{enumerate}
	...
	\item In the \fmtRibbonButton{Find} box, type $ 3.4 $, and then click \fmtPopupButton{Find Next}.
\end{enumerate}
...
\begin{enumerate}
	\item Click the \fmtPopupButton{Close} button.
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item In the \fmtRibbonTab{Home} tab of the ribbon, click on \fmtRibbonButton{Find \& Select} in the \fmtRibbonGroup{Editing} Group and then click \fmtPopupButton{Find}.
			\item In the Find box, type the phrase to find then click \fmtPopupButton{Find Next}.
			\item Continue clicking \fmtPopupButton{Find Next} until the phrase is found.
			\item Click \fmtPopupButton{Close} and edit the data.
...
\begin{enumerate}
	\item Click in \fmtCellLocation{A6}, the left-most cell \textit{below} the headings row.
	\item Click the \fmtRibbonTab{View} tab in the ribbon.
	\item Select \fmtRibbonButton{Freeze Panes} and then \fmtRibbonButton{Freeze Top Row} (see Figure \ref{05:fig07}).
...
\begin{enumerate}
	\item Click on the \fmtRibbonTab{View} tab in the ribbon.
	\item Select \fmtRibbonButton{Unfreeze Panes}.
\end{enumerate}
...
\begin{enumerate}
	\item Click on the filter \fmtRibbonButton{Click arrow} to the right of the header \textit{Snow (inches)} in the \fmtWorksheetName{Portland ME} worksheet.
...
\begin{enumerate}
	\item Now switch to the \fmtWorkbookName{Portland OR} sheet and repeat these sort steps to find the snowiest day in Oregon. Check the worksheet with Figure \ref{05:fig10}.
\end{enumerate}
...
\begin{enumerate}
	\item Click on the \fmtWorksheetName{Portland OR} sheet then click on a cell in the table.
...
In the weather data, a column was added for the week in the \fmtWorksheetName{Weekly OR} sheet and changed the days to Sunday through Saturday. This sheet facilitates further analysis of Portland, Oregon's data to see if there are weekly trends in the weather. To look for those trends, sort the \fmtWorksheetName{Weekly OR} sheet by Week and then by Day.

\begin{enumerate}
	\item Click on the \fmtWorksheetName{Weekly OR} worksheet.
\item Click on \fmtCellLocation{A5} and insert a table.
\item Click on \fmtRibbonButton{Sort} in the \fmtRibbonTab{Data} tab in the ribbon.
\item Click the down-arrow for Column and select \fmtPopupButton{Week}.
\item Click the down-arrow for Order and select \fmtPopupButton{Smallest to Largest}.
\item To add 2nd level sort, click on the \fmtPopupButton{Add Level} button in the top right corner of the dialog box.
\item In the new \fmtPopupBox{Then by} row, click the down-arrow for Column and select \fmtPopupButton{Day}.
\item Click the down-arrow for Order and select \fmtPopupButton{Custom List}. The dialog box in Figure \ref{05:fig13} will appear on the screen.
\end{enumerate}
...
\begin{enumerate}[resume]
	\item Click on \fmtPopupButton{Sunday, Monday, Tuesday, etc.} in the \fmtPopupBox{Custom} lists on the left-side of the dialog box. NOTE: Make sure to select the days of the week spelled out, not the abbreviations for the days of the week.
	\item Click \fmtPopupButton{OK}. The \fmtPopupBox{Sort} dialog box should look like Figure \ref{05:fig14}.
\end{enumerate}
...
\begin{enumerate}
	\item Click \fmtPopupButton{OK} again. The sorted table should now look like Figure \ref{05:fig15}. Notice the data is in Week order and, within each week, in Day order.
...
\begin{center}
	\begin{tkwbox}{Key Take-Aways}
		...
		\begin{itemize}
			...
			\item Tables are created by clicking in the top left-most cell in the data and selecting \fmtRibbonButton{Table} in the \fmtRibbonTab{Insert} tab of the ribbon.
			...
			\item Filter arrows in the table headings sort the data by a single column. Use \fmtRibbonButton{Sort} in the \fmtRibbonTab{Data} tab in the ribbon to sort by two or more columns at a time.
...
\begin{table}[H]
	...
	{\small
		%\fontsize{8}{10} \selectfont %Replace small for special font size
		\begin{longtable}{L{1.50in}L{0.75in}L{1.00in}L{0.75in}} %Left-aligned, Max width: 4.25in
...
Notice there is sometimes more than one way to filter data (\ie with a filter choice or a checked box). There are also single criteria filters and multi-criteria filters. To start filtering, look at just the first week of data in the \fmtWorksheetName{Weekly OR} sheet.

\begin{enumerate}
	\item Click on the \fmtWorksheetName{Weekly OR} sheet and click on a cell in the table.
	\item Click the filter arrow to the right of the Week heading.
	\item Click the \fmtPopupButton{Select All} checkbox to deselect all of the checkbox choices.
...
\begin{enumerate}
	...
	\item Select \fmtPopupButton{Clear filter from Week}.
\end{enumerate}
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Select \fmtPopupButton{Clear filter}.
...
\begin{enumerate}
	\item Click in the \fmtWorksheetName{Portland ME} sheet, then click on a cell in the table.
	\item Click on the filter arrow next to the High heading.
	\item Click on \fmtPopupButton{Number} filters, then select \fmtPopupButton{Greater than}. The \fmtPopupBox{Custom AutoFilter} dialog box will appear on the screen.
...
\begin{enumerate}
	\item Click on the \fmtWorksheetName{Weekly OR} sheet and clear the Day column filter.
...
\begin{enumerate}
	\item Click on the \fmtWorksheetName{Portland OR} sheet and click in the table.
	\item In the ribbon's \fmtRibbonTab{Table Tools Design} tab, click \fmtRibbonButton{Insert Slicer}.
	\item Click on \fmtPopupButton{Day} in the \fmtPopupBox{Insert Slicers} dialog box, and then click \fmtPopupButton{OK}.
	\item Drag the slicer so that the upper left-hand corner lines up with the top corner of cell \fmtCellLocation{G5}.
	...
	\item Click on the \fmtPopupButton{Slicer options} tab, then click on the \fmtPopupButton{More} button next to Slicer Styles. The choices in Figure \ref{05:fig20} will show on the screen.
\end{enumerate}
...
\begin{enumerate}
	...
	\item In the \fmtRibbonGroup{Size} group on the \fmtRibbonTab{Slicer Options} ribbon (\textit{NOT} the \fmtRibbonGroup{Buttons} group), change the width to $ 1 $ inch.
...
\begin{enumerate}
	\item Click on the \fmtWorksheetName{Portland ME} sheet and clear the filter from the High column.
	\item Click on the \fmtRibbonButton{Total Row} check box in the \fmtRibbonGroup{Table Style Options} group in the \fmtRibbonTab{Table Tools Design} tab in the ribbon.
	\item Scroll to the bottom of the table to the Total Row. Notice the total for the Snow data.
	\item Click on \fmtCellLocation{D37} (in the Rain column), and then click the down-arrow that appears to the right of the cell.
	\item Choose \fmtRibbonButton{Sum} to add a sum to the Total Row in the Rain column.
	\item To see the Average rainfall for the month of January, click on the arrow again and choose \fmtRibbonButton{Average}.
	\item Repeat this step in \fmtCellLocation{E37} to see the Average snowfall.
	\item Use the \fmtRibbonButton{Decrease Decimal} button in the \fmtRibbonTab{Home} tab of the ribbon to change the decimal places in \fmtCellLocation{D37} and \fmtCellLocation{E37} to $ 2 $. Compare the Total Row to Figure \ref{05:fig22}.
\end{enumerate}
...
Now switch to the \fmtWorksheetName{Weekly OR} sheet and add a Slicer and Total Row to this table.

\begin{enumerate}
	\item Open the \fmtWorksheetName{Weekly OR} sheet.
	\item Clear the filter from the Day column.
	\item Add a Slicer for the Day column to the sheet.
	\item Move the top left corner of the slicer to \fmtCellLocation{H5}. Resize it as needed and choose a Slicer Style.
	\item Select Monday through Friday in the Slicer so that Saturday and Sunday data do \textit{NOT} show in the table.
	\item Add a Total Row that averages the High and Low columns. The averages should be High: $ 47.0 $ and Low: $ 35.8 $. Change the label \textit{Total} to \textit{Average} by clicking \fmtCellLocation{A37} and typing \fmtTyping{Average}.
\end{enumerate}
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Choose \fmtPopupButton{Sum} to add a sum to the Total Row in the column.
			\item To see the Average for column, click on the arrow again and choose \fmtPopupButton{Average}. Some other choices in the Total Row are Count (for words), Count Numbers, Max, and Min.
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Click on \fmtRibbonButton{Insert Slicer} in the \fmtRibbonTab{Table Tools Design} tab in the ribbon.
...
\begin{enumerate}
	...
	\item Click \fmtRibbonButton{Subtotal} in the \fmtRibbonTab{Data} tab in the ribbon.
	\item To limit the displayed data further, click \fmtRibbonButton{Filter} in the \fmtRibbonTab{Data} tab in the ribbon.
\end{enumerate}
...
\begin{enumerate}
	\item Click on the \fmtWorksheetName{Weekly OR} sheet.
\item Point at the \fmtWorksheetName{Weekly OR} sheet tab at the bottom of the screen, hold the \fmtKeystroke{Ctrl} key down, and left-drag the sheet to the right until it is past all the existing sheets.
\item When a sheet icon with a \textbf{+} sign is visible, let go of the mouse button and then the \fmtKeystroke{Ctrl} key. A \fmtWorksheetName{Weekly OR (2)} sheet will appear.
\item Right-click on the new sheet tab, select \fmtPopupButton{Rename}, type \fmtTyping{Subtotal OR}, and then press \fmtKeystroke{Enter}.
\item Save the file.
\item Remove all filters in the table by clicking the \fmtRibbonTab{Data} tab and then choosing \fmtRibbonButton{Clear}.
\item Now Sort the table by the Day column using a \fmtPopupButton{Custom Sort} in the \fmtRibbonButton{Sort} button in the ribbon to sort in the order Sunday, Monday, Tuesday, etc. (See Figure \ref{05:fig13} through Figure \ref{05:fig15} for a review of Custom Sorting.)
\item Before adding subtotals, convert the table back to a regular range. To do this, click \fmtRibbonButton{Convert to Range} in the \fmtRibbonTab{Table Tools Design} tab on the ribbon. (See Figure \ref{05:fig23})
\end{enumerate}
...
\begin{enumerate}[resume]
	\item When a box pops up with a warning about converting the table, click \fmtPopupButton{Yes}.
	\item Because the data is no longer formatted as a table, the slicer will disappear and the \fmtRibbonTab{Table Tools Design} tab in the ribbon will no longer be available.
	\item Under the \fmtRibbonTab{Data} tab in the ribbon, click \fmtRibbonButton{Subtotal}.
	\item In the \fmtPopupBox{Subtotal Window}, make the choices shown in the Figure \ref{05:fig24}. It is essential to select the column sorted by in the \fmtPopupBox{At each change in field} at the top of the window. Click \fmtPopupButton{OK}.
\end{enumerate}
...
Notice the three \fmtPopupButton{Outline} buttons circled in the upper-left corner of the spreadsheet. These control the amount of subtotaled data that is displayed. Table \ref{05:tab06} describes the different \fmtPopupButton{Outline} buttons.
...
Click the three \fmtPopupButton{Outline} buttons to see the difference in the data displayed.

\begin{enumerate}
	\item Click on the \fmtPopupButton{$ 1 $ Outline} button in the upper left-hand corner of the sheet.
	\item Only the Grand Average row with averages for High, Low, Rain, and Snow should be visible.
	\item Click on the \fmtPopupButton{$ 2 $ Outline} button.
	\item The average for each day of the week along with the Grand Average are now visible.
	\item Click on the \fmtPopupButton{+ Sign} button to the left of the Sunday Average row.
	\item This expands just the Sunday Day data and displays the individual records for this subset of the data. Clicking on \fmtPopupButton{+ Sign} buttons will expand a portion of the data at a time. Clicking on \fmtPopupButton{– Sign} buttons hide a portion of the data at a time.
	\item Click on the \fmtPopupButton{$ 3 $ Outline} button.
...
\begin{center}
	\begin{tkwbox}{Key Take-Aways}
		...
		\begin{itemize}
			...
			\item \fmtPopupButton{Subtotal Outline} buttons allow users to see add of the subtotaled data, just the totals and grand total, or simply the grand total.
			\item \fmtPopupButton{+ and - Buttons} within subtotaling allow a user to expand and hide portions of the subtotaled data.
...
Now that the weather data has been sorted, filtered, and subtotaled as needed, it is time to print the worksheets. Start with the \fmtWorksheetName{Portland ME} worksheet.

\begin{enumerate}
	\item Click on the \fmtWorksheetName{Portland ME} worksheet. 
	\item If needed, use \fmtKeystroke{Ctrl}+\fmtKeystroke{Home} to move to cell \fmtCellLocation{A1}.
\end{enumerate}

Notice that cells \fmtCellLocation{A1}, \fmtCellLocation{A2}, and \fmtCellLocation{A3} are not merged and centered over the entire table of data. To fix this, unmerge each of the merged cells, and then merge them again, making sure to include \fmtCellLocation{E1}, \fmtCellLocation{E2}, and \fmtCellLocation{E3} in the selection.

\begin{enumerate}
	\item Select cell \fmtCellLocation{A1} and click the \fmtRibbonButton{Merge \& Center} button. This should split \fmtCellLocation{A1} into four cells (\fmtCellLocation{A1:D1}).
	\item Select the range \fmtCellLocation{A1:E1} and click the \fmtRibbonButton{Merge \& Center} button. Cell \fmtCellLocation{A1} should now be merged across \fmtCellLocation{A1:E1}.
	\item Repeat these steps for \fmtCellLocation{A2} and \fmtCellLocation{A3}.
\end{enumerate}

Next preview the worksheet in \fmtPopupBox{Print Preview} and determine what page setup options need to be set.

\begin{enumerate}
	\item Go to Backstage view and select \fmtRibbonButton{Print} from the menu.
\end{enumerate}
...
\begin{enumerate}
	\item In the \fmtPopupBox{Settings} section, click the link for \fmtPopupButton{Page Setup}. This opens the \fmtPopupBox{Page Setup} dialog box. See Figure \ref{05:fig26}.
\end{enumerate}
...
\begin{enumerate}[resume]
	...
	\item In the \fmtPopupBox{Center on page} section, check the box for \fmtPopupButton{Horizontally}.
	\item Click \fmtPopupButton{OK}. The table should now be centered horizontally on the page.
	\item Next, add a footer with the workbook filename and worksheet name.
	\item Open the \fmtPopupBox{Page Setup} dialog box again.
	\item Click the \fmtPopupButton{Header/Footer} tab then click the \fmtPopupButton{Custom Footer} button.
	\item In the \fmtPopupBox{Left section:} box type \fmtTyping{File:}.
	\item Making sure to leave a space after the colon, click the \fmtPopupButton{Insert File Name} button.
	\item In the \fmtPopupBox{Right section:} box type \fmtTyping{Worksheet:}.
	\item Making sure to leave a space after the colon, click the \fmtPopupButton{Insert Sheet Name} button.
	\item The Footer dialog box should look like Figure \ref{05:fig27}. Click the \fmtPopupButton{OK} button twice to return to Print Preview. Confirm that the footer appears correctly, then exit Backstage View.
\end{enumerate}
...
\begin{enumerate}
	\item Click the \fmtRibbonTab{Insert} tab on the ribbon.
	\item Click the \fmtRibbonButton{Pictures} button from the \fmtRibbonGroup{Illustrations} group. (This inserts an image from the local computer. To search for an online image, click the \fmtRibbonButton{Online Pictures} button.) (See Figure \ref{05:fig28}.)
\end{enumerate}
...
The image now appears on the worksheet, but not in the desired location. It is also slightly larger than desired. (See Figure \ref{05:fig29}.) Move the image to cell \fmtCellLocation{E1}, then resize it so it does not cover up part of the table.
...
\begin{enumerate}
	\item Place the pointer in the image so that the the pointer changes to crosshairs. Drag the image so that the top left corner is in cell \fmtCellLocation{E1}.
	...
	\item Check \fmtPopupBox{Print Preview} again to make sure the worksheet with the image added looks good.
...
\begin{enumerate}
	\item Click the \fmtWorksheetName{Portland OR} worksheet and go to \fmtPopupBox{Print Preview}. No changes need to be made to this worksheet. Exit Backstage View.
	\item Click the \fmtWorksheetName{Weekly OR} worksheet and go to \fmtPopupBox{Print Preview}. Notice that the Slicer is printing on a second page. To fix this, set the \fmtPopupBox{Page Scaling} to \fmtPopupButton{Fit All Columns on One Page}.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Return to \fmtPopupBox{Print Preview} and confirm the worksheet, including the slicer, is printing appropriately. Exit Backstage View.
	\item Click the \fmtWorksheetName{Subtotal OR} worksheet and go to \fmtPopupBox{Print Preview}.
	\item Using the \fmtPopupBox{Page Setup} dialog box, center this worksheet horizontally on the page. 
	\item Exit Backstage View.
	\item Save the \fmtWorkbookName{CH5 National Weather} workbook.
	\item Compare the worksheet with the self-check answer key (found in the Course Files) and then submit the \fmtWorkbookName{CH5 National Weather} workbook as directed by the instructor.
\end{enumerate}
...
\begin{center}
	\begin{tkwbox}{Key Take-Aways}
		...
		\begin{itemize}
			...
			\item When working with Excel workbooks, the final step should always be to review the worksheets in \fmtPopupBox{Print Preview} to make sure they are printing appropriately.
...
\begin{enumerate}
	\item Open the data file \fmtWorkbookName{PR5 Data} and save the file as \fmtWorkbookName{PR5 Canyon Trails}.
	\item In Column J, calculate Total Cost (number of \fmtTyping{Guests*Per Person Cost}). Copy the formula down the column.
	\item Format Columns I and J with Currency and no decimal places.
	\item Center all headings in Row 3.
	\item Click in cell \fmtCellLocation{A3}. Insert a table with headers for the range \fmtCellLocation{A3:J53}.
	\item Adjust column widths within the table so that all the headings are completely visible.
	\item Rename \fmtWorksheetName{Sheet $ 1 $} to \fmtWorksheetName{Current Tours}. Sort this sheet alphabetically (A to Z) by Last Name.
	\item Make a copy of the \fmtWorksheetName{Current Tours} sheet and rename it \fmtWorksheetName{Tours by Canyon}. Place the \fmtWorksheetName{Tours by Canyon} sheet to the right of the \fmtWorksheetName{Current Tours} sheet. Sort this sheet by Tour Canyon (A to Z), then Home Country (A to Z), and then Last Name (A to Z).
	\item Make another copy of the \fmtWorksheetName{Current Tours} sheet and rename it \fmtWorksheetName{US Guests}. Place the \fmtWorksheetName{US Guests} sheet to the right of the \fmtWorksheetName{Tours by Canyon} sheet. Filter this sheet so that only guests with a Home Country of the United States show. Sort the filtered data alphabetically (A to Z) by Tour State. Add a Total Row that sums the Guests and Total Cost columns.
	\item Make another copy of the \fmtWorksheetName{Current Tours} sheet and rename it \fmtWorksheetName{European Guests}. Place the \fmtWorksheetName{European Guests} sheet to the right of the \fmtWorksheetName{US Guests} sheet. Hide the Average Age column.
	\item Insert a slicer in the \fmtWorksheetName{European Guests} sheet for Home Country. Move the top left corner of the slicer to the top left-hand corner of cell \fmtCellLocation{K3}. Change the width of the entire slicer to $ 1.65 $ inches.
	...
	\item Make one more copy of the \fmtWorksheetName{Current Tours} sheet and rename it \fmtWorksheetName{Tours by State}. Place the \fmtWorksheetName{Tours by State} sheet to the right of the \fmtWorksheetName{European Guests} sheet. Subtotal the sheet by State, summing the Total Cost column.
	\item Change the name of the \fmtWorksheetName{Tours by State} sheet to \fmtWorksheetName{$ 5-7 $ Day Tours by State}. Filter out $ 3 $ day tours in the table.
	...
	\item Save the \fmtWorkbookName{PR5 Canyon Trails} workbook.
	\item Make sure the sheets are in the following order from left to right: \fmtWorksheetName{Current Tours}, \fmtWorksheetName{Tours by Canyon}, \fmtWorksheetName{US Guests}, \fmtWorksheetName{European Guests}, and \fmtWorksheetName{$ 5-7 $ Day Tours by State}.
	\item Compare the workbook with the self-check answer key (found in the Course Files) and then submit the \fmtWorkbookName{PR5 Canyon Trails} workbook as directed by the instructor.
...
\begin{enumerate}
	\item 1. Open the data file \fmtWorkbookName{SC5 Data} and save the file as \fmtWorkbookName{SC5 Dynamite Customer Sales}.
	\item Click on the \fmtWorksheetName{Sales} sheet. In \fmtCellLocation{I4}, enter a \fmtTyping{Vlookup} function that will find the Product Price for the Product in \fmtCellLocation{E4} in the table in the \fmtWorksheetName{Product Table} sheet and return that price to \fmtCellLocation{I4}. In the \fmtTyping{Vlookup} function, fill in the required parameters using Figure \ref{05:fig32} below. Copy the \fmtTyping{Vlookup} function down column I.
\end{enumerate}
...
\begin{enumerate}[resume]
	\item In \fmtCellLocation{J4} , enter a \fmtTyping{Vlookup} function that will find the Product Cost for the Product in \fmtCellLocation{E4} in the table in the \fmtWorksheetName{Product Table} sheet and return it to \fmtCellLocation{J4}. This \fmtTyping{Vlookup} function will be the same as the \fmtTyping{Vlookup} function in \fmtCellLocation{I4} \textit{EXCEPT} the COL\_INDEX\_NUM will be $ 3 $ instead of $ 4 $. Copy the function down column J.
	\item In \fmtCellLocation{K4}, calculate Profit (Product Price – Product Cost). Copy this formula down column K.
	\item Format columns I, J, and K as currency with two decimal places.
	\item Click in cell \fmtCellLocation{A3}. Insert a table with headers for the range \fmtCellLocation{A3:K52}. \textit{BE CAREFUL HERE}: Excel will try to insert a table starting with \fmtCellLocation{A2}. Be certain the range starts with \fmtCellLocation{A3} here.
	\item Make a copy of the \fmtWorksheetName{Sales} sheet and rename it \fmtWorksheetName{Online Sales by Date}. Place this sheet to the right of the \fmtWorksheetName{Sales} sheet. Filter out Retail in Sales Type, so that only Online Sales are displayed. Sort the filtered data by Date Sold (oldest to newest).
	\item Make a copy of the \fmtWorksheetName{Sales} sheet and rename it \fmtWorksheetName{June Sales by Country}. Place this new sheet to the right of the \fmtWorksheetName{Online Sales by Date} sheet. Filter this sheet to only show June dates by using the Date Filter Between. Sort this sheet alphabetically (A to Z) by Country and then alphabetically by Name.
	\item Make another copy of the \fmtWorksheetName{Sales} sheet and rename it \fmtWorksheetName{Sales by Product}. Place this new sheet to the right of the \fmtWorksheetName{June Sales by Country} sheet. Hide the Region column.
	\item Insert a slicer in the \fmtWorksheetName{Sales by Product} sheet for Product Sold. Move the top left corner of the slicer to the top left-hand corner of cell \fmtCellLocation{M1}. Resize the height of the entire slicer to $ 2.09 $ inches.
	\item Select both DETA100 and DETA200 in the slicer. Sort the filtered sheet by Product Sold. Add a Total Row that includes the overall average for the Product Price, Product Cost, and Profit columns. Change the heading in \fmtCellLocation{A53} to \fmtTyping{Average}.
	\item Make a copy of the \fmtWorksheetName{Sales} sheet and rename it \fmtWorksheetName{Subtotals by Date}. Place this new sheet to the right of the \fmtWorksheetName{Sales by Product} sheet. Subtotal the sheet by Date (Oldest to Newest), summing the Profit column. Click the \fmtPopupButton{2 Outline} button to show just the subtotals by date and the grand total.
	\item Make a copy of the \fmtWorksheetName{Sales} sheet and rename it \fmtWorksheetName{Subtotals by Type}. Place this new sheet to the right of the \fmtWorksheetName{Subtotals by Date} sheet. Subtotal the sheet by Sales Type, summing the Profit column.
	\item Add a second subtotal to the \fmtWorksheetName{Subtotals by Type} sheet that subtotals by Type and averages the Profit column. (\textit{Hint}: uncheck Replace Current Subtotals in the Subtotal dialog box.) Notice that four outline buttons appear with the second subtotal. Figure out which Outline button to click to display both subtotals for Online and Retail and two Grand Totals.
	...
	\item Double-check that the sheets are in the following order from left to right: \fmtWorksheetName{Sales}, \fmtWorksheetName{Online Sales by Date}, \fmtWorksheetName{June Sales by Country}, \fmtWorkbookName{Sales by Product}, \fmtWorksheetName{Subtotals by Date}, \fmtWorksheetName{Subtotals by Sales Type}, and \fmtWorksheetName{Product Table}.
	\item Save the \fmtWorkbookName{SC5 Dynamite Customer Sales} workbook.
	\item Submit the \fmtWorkbookName{SC5 Dynamite Customer Sales} workbook as directed by the instructor.
\end{enumerate}
