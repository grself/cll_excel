...
\begin{table}[H]
	...
	{\small		
		\begin{longtable}{L{1.0in}L{3.00in}} %Max width: 4.25in
...
\begin{enumerate}
	\item Click in \fmtCellLocation{A6} which is the left-most cell below the headings row.
...
\begin{enumerate}
	\item Click on the arrow to the right of the Snow (inches) header in the \fmtWorksheetName{Portland ME} worksheet.
	\item Click on the choice \fmtButton{Click ZA$ \downarrow $ Sort Largest to Smallest}. See Figure \ref{05:fig08}.
\end{enumerate}
...
If this is done correctly, the snowiest day, January 3rd (in \fmtCellLocation{row 6}) with $ 0.73 $ inches of snow, will be at the top of the list. Notice the filter arrow changes in the snow column to a downward pointing arrow to indicate that the column is sorted in descending order (largest to smallest).
...
\begin{enumerate}
	\item Now switch to the \fmtWorksheetName{Portland OR} sheet and repeat these sort steps to find the snowiest day in Oregon. Check the worksheet with Figure \ref{05:fig10}.
\end{enumerate}
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Click on the choice \fmtButton{AZ$ \downarrow $} or \fmtButton{ZA$ \downarrow $} to sort the data in that column.
...
\begin{enumerate}
	\item Click on the \fmtWorksheetName{Portland OR} sheet then click on any cell in the table.
	\item Click on the \fmtRibbonTab{Data} tab in the ribbon and then click the \fmtButton{Sort} button.
	\item Click the down-arrow for Column and select \fmtButton{Snow (inches)}.
	\item Click the down-arrow for Order and select \fmtButton{Largest to Smallest}.
	\item To add 2nd level sort, click on the \fmtButton{Add Level} button in the top left corner of the dialog box.
	\item In the new ``Then by'' row, click the down-arrow for Column and select \fmtButton{Low (°F)}.
	\item In the same row, click the down-arrow for Order and select \fmtButton{Smallest to Largest}. The dialog box should look like Figure \ref{05:fig11}.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Click on \fmtButton{Sort} in the \fmtRibbonTab{Data} tab in the ribbon.
	\item Click the down-arrow for Column and select \fmtButton{Week}.
	\item Click the down-arrow for Order and select \fmtButton{Smallest to Largest}.
	\item To add 2nd level sort, click on the \fmtButton{Add Level} button in the top right corner of the dialog box.
	\item In the new ``Then by'' row, click the down-arrow for Column and select \fmtButton{Day}.
	\item Click the down-arrow for Order and select \fmtButton{Custom List}. The dialog box in Figure \ref{05:fig13} will appear on the screen.
\end{enumerate}
...
\begin{enumerate}[resume]
	\item Click on \fmtButton{Sunday, Monday, Tuesday, etc.} in the Custom lists on the left-side of the dialog box. NOTE: Make sure to select the days of the week spelled out, not the abbreviations for the days of the week.
	\item Click \fmtButton{OK}. The Sort dialog box should look like Figure \ref{05:fig14}.
\end{enumerate}
...
\begin{enumerate}
	\item Click \fmtButton{OK} again. The sorted table should now look like Figure \ref{05:fig15}. Notice the data is in Week order and, within each week, in Day order.
...
Notice there are sometimes more than one way to filter data (\ie with a filter choice or a checked box). There are also single criteria filters, as well as multi-criteria filters. The first week of data in the \fmtWorksheetName{Weekly OR} sheet is used to introduce filtering.
...
\begin{enumerate}
	...
	\item Select \fmtButton{Clear filter} from Week.
\end{enumerate}
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Select \fmtButton{Clear filter}.
...
\begin{enumerate}
	\item Click in the \fmtWorksheetName{Portland ME} sheet, then click on any cell in the table.
	\item Click on the filter arrow next to the High heading.
	\item Click on \fmtButton{Number} filters, then select \fmtButton{Greater than}. The Custom AutoFilter dialog box will appear on the screen.
...
\begin{enumerate}
	\item Click on the \fmtButton{Weekly OR} sheet and clear the Day column filter.
...
\begin{enumerate}
	\item Click on the \fmtWorksheetName{Portland OR} sheet and click any cell in the table.
	\item In the ribbon's \fmtRibbonTab{Table Tools Design} tab, click \fmtButton{Insert Slicer}.
	\item Click on Day in the Insert Slicers dialog box, and then click \fmtButton{OK}.
	\item Drag the slicer so that the upper left-hand corner lines up with the top corner of cell \fmtCellLocation{G5}.
	\item Notice that when a Slicer is inserted, a \fmtRibbonTab{Slicer Options} tab appears on the ribbon. This tab contains links to change the style and size of the entire slicer or individual slicer buttons.
	\item Click on the \fmtRibbonTab{Slicer options} tab, then click on the \fmtButton{More} button next to \fmtRibbonGroup{Slicer Styles}. The choices in Figure \ref{05:fig20} will show on the screen.
\end{enumerate}
...
\begin{enumerate}
	...
	\item In the \fmtRibbonGroup{Size group} on the \fmtRibbonTab{Slicer Options} ribbon (NOT the \fmtRibbonGroup{Buttons} group), change the width to $ 1 $ inch.
...
\begin{enumerate}
	...
	\item Click on the \fmtButton{Total Row} check box in the \fmtRibbonGroup{Table Style Options} group in the \fmtRibbonTab{Table Tools Design} tab in the ribbon.
	\item Scroll to the bottom of the table to the \fmtCellLocation{Total Row}. Notice the total for the Snow data.
	\item Click on \fmtCellLocation{D37} (in the Rain column), and then click the down-arrow that appears to the right of the cell.
	\item Choose \fmtButton{Sum} to add a sum to the \fmtCellLocation{Total Row} in the Rain column.
	\item To see the Average rainfall for the month of January, click on the arrow again and choose \fmtButton{Average}.
	\item Repeat this step in \fmtCellLocation{E37} to see the Average snowfall.
	\item Use the \fmtButton{Decrease Decimal} button in the \fmtRibbonTab{Home} tab of the ribbon to change the decimal places in \fmtCellLocation{D37} and \fmtCellLocation{E37} to $ 2 $. Compare the \fmtCellLocation{Total Row} to Figure $ 5.22 $.
\end{enumerate}
...
Now switch to the \fmtWorksheetName{Weekly OR} sheet and add a \fmtButton{Slicer} and \fmtCellLocation{Total Row} to this table.

\begin{enumerate}
	\item Open the \fmtWorksheetName{Weekly OR} sheet.
	\item Clear the filter from the \fmtCellLocation{Day} column.
	\item Add a \fmtButton{Slicer} for the \fmtCellLocation{Day} column to the sheet.
	...
	\item Add a \fmtCellLocation{Total Row} that averages the High and Low columns. The averages should be High: $ 47.0 $ and Low: $ 35.8 $. Change the label \textit{Total} to \textit{Average} by clicking \fmtCellLocation{A37} and typing \textit{Average}.
\end{enumerate}
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Click on the \fmtButton{Total Row} check box in the \fmtRibbonGroup{Table Style Options} group in the \fmtRibbonTab{Table Tools Design} tab in the ribbon.
			\item Scroll to the bottom of the table to find the \fmtCellLocation{Total Row}.
			\item Click in one of the columns in the \fmtCellLocation{Total Row}, and then click the down-arrow that appears to the right of the cell.
			\item Choose \fmtButton{Sum} to add a sum to the \fmtCellLocation{Total Row} in the column.
			\item To see the Average for column, click on the arrow again and choose \fmtButton{Average}. Some other choices in the Total Row are Count (for words), Count Numbers, Max, and Min.
...
\begin{enumerate}
	...
	\item \fmtRibbonButton{Subtotal} in the \fmtRibbonTab{Data} tab in the ribbon.
	\item To limit the displayed data further, \fmtRibbonButton{Filter} in the \fmtRibbonTab{Data} tab in the ribbon.
\end{enumerate}
...
\begin{enumerate}
	...
	\item When a sheet icon with a $ + $ sign is visible, let go of the mouse button and then the \fmtKeystroke{Ctrl} key. A \fmtWorksheetName{Weekly OR (2)} sheet will appear.
	\item Right-click on the new Sheet tab, select \fmtButton{Rename}, type \textit{Subtotal OR}, and then press \fmtKeystroke{Enter}.
	\item Save the file.
	\item Remove all filters in the table by clicking the \fmtRibbonTab{Data} tab and then choosing \fmtRibbonButton{Clear}.
	\item Now Sort the table by the Day column using a Custom Sort in the \fmtRibbonButton{Sort} button in the ribbon to sort in the order Sunday, Monday, Tuesday, etc. (See Figure \ref{05:fig13} through Figure \ref{05:fig15} for a review of Custom Sorting.)
...
\begin{enumerate}[resume]
	...
	\item In the Subtotal Window, make the choices shown in the Figure \ref{05:fig24}. It is essential to select the column sorted by in the ``At each change in'' field at the top of the window. Click \fmtButton{OK}.
\end{enumerate}
...
Notice the three \fmtButton{Outline} buttons circled in the upper-left corner of the spreadsheet. These control the amount of subtotaled data that is displayed. Table \ref{05:tab06} describes the different \fmtButton{Outline} buttons.

\begin{table}[H]
	\rowcolors{1}{}{tablerow} % zebra striping background
	{\small		
		\begin{longtable}{p{0.50in}p{3.00in}} %Left-aligned, Max width: 4.25in
...
Try the three \fmtButton{Outline} buttons to see the difference in the data displayed.
...
\begin{enumerate}
	...
	\item Repeat steps $ 1 $ and $ 2 $ for \fmtCellLocation{A2} and \fmtCellLocation{A3}.
\end{enumerate}
...
\begin{enumerate}
	\item In the \fmtRibbonGroup{Settings} section, click the link for \fmtRibbonButton{Page Setup}. This opens the Page Setup dialog box. See Figure \ref{05:fig26}.
\end{enumerate}
...
\begin{enumerate}[resume]
	\item Click on the \fmtRibbonTab{Margins} tab.
	\item In the \fmtRibbonGroup{Center on page} section, check the box for \fmtRibbonButton{Horizontally}.
	...
	\item Open the \fmtRibbonTab{Page Setup} dialog box again (see Step $ 1 $ above).
	\item Click the \fmtRibbonTab{Header/Footer} tab then click the \fmtRibbonButton{Custom Footer} button.
	\item In the Left section box, type \textit{File:}.
	\item Making sure to leave a space after the colon, click the \fmtRibbonButton{Insert File Name} button.
	\item In the Right section box, type \textit{Worksheet:}.
	\item Making sure to leave a space after the colon, click the \fmtRibbonButton{Insert Sheet Name} button.
	\item The Footer dialog box should look like Figure \ref{05:fig27}. Click the \fmtButton{OK} button twice to return to Print Preview. Confirm that the footer appears correctly, then exit Backstage View.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Click the \fmtRibbonButton{Pictures} button from the \fmtRibbonGroup{Illustrations} group. (This inserts an image from a file on the local computer. To search for an online image, click the \fmtRibbonButton{Online Pictures} button.) (See Figure \ref{05:fig28}.)
\end{enumerate}
...
\begin{enumerate}
	\item Click the \fmtWorksheetName{Portland OR} worksheet and go to \fmtRibbonButton{Print Preview}. No changes need to be made to this worksheet. Exit Backstage View.
	\item Click the \fmtWorksheetName{Weekly OR} worksheet and go to \fmtRibbonButton{Print Preview}. Notice that the Slicer is printing on a second page. To fix this, set the Page Scaling to \fmtRibbonButton{Fit All Columns on One Page}.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Click the \fmtWorksheetName{Subtotal OR} worksheet and go to \fmtRibbonButton{Print Preview}.
...
\begin{enumerate}
	...
	\item In \fmtCellLocation{Column J}, calculate Total Cost (\fmtFunction{number of Guests * Per Person Cost}). Copy the formula down the column.
	\item Format \fmtCellLocation{Columns I, J} with Currency and no decimal places.
	\item Center all headings in \fmtCellLocation{Row 3}.
	...
	\item Rename Sheet $ 1 $ \fmtWorksheetName{Current Tours}. Sort this sheet alphabetically (A to Z) by Last Name.
...
Retail companies with online and in-store sales need to keep track of extensive data. Keeping track of sales, costs, and profits on a daily basis is essential to making the most of a business. This exercise illustrates how to use the skills presented in this chapter to generate the data needed on a daily basis by a retail company. See Figure \ref{05:fig31} above.

\begin{enumerate}
	\item 1. Open the data file \fmtWorkbookName{SC5 Data} and save the file as \fmtWorkbookName{SC5 Dynamite Customer Sales}.
	\item Click on the \fmtWorksheetName{Sales} sheet. In \fmtCellLocation{I4}, enter a \fmtFunction{Vlookup} function that will find the Product Price for the Product in \fmtCellLocation{E4} in the table in the \fmtWorksheetName{Product Table} sheet and return it to \fmtCellLocation{I4}. In the \fmtFunction{Vlookup} function, fill in the required parameters using Figure \ref{05:fig32} below. Copy the \fmtFunction{Vlookup} function down \fmtCellLocation{column I}.
\end{enumerate}
...
\begin{enumerate}[resume]
	\item In \fmtCellLocation{J4} , enter a \fmtFunction{Vlookup} function that will find the Product Cost for the Product in \fmtCellLocation{E4} in the table in the \fmtWorksheetName{Product Table} sheet and return it to J4. This \fmtFunction{Vlookup} function will be the same as the \fmtFunction{Vlookup} function in \fmtCellLocation{I4} EXCEPT the COL\_INDEX\_NUM will be $ 3 $ instead of $ 4 $. Copy the function down \fmtCellLocation{Column J}.
	\item In \fmtCellLocation{K4}, calculate Profit (\fmtFunction{Product Price – Product Cost}). Copy this formula down \fmtCellLocation{Column K}.
	\item Format \fmtCellLocation{Columns I, J, and K} as currency with two decimal places.
	\item Click in cell \fmtCellLocation{A3}. Insert a table with headers for the range \fmtCellLocation{A3:K52}. BE CAREFUL HERE: Excel will try to insert a table starting with \fmtCellLocation{A2}. Be certain the range starts with \fmtCellLocation{A3} here.
	...
	\item Select both DETA100 and DETA200 in the slicer. Sort the filtered sheet by Product Sold. Add a Total Row that includes the overall average for the Product Price, Product Cost, and Profit columns. Change the heading in \fmtCellLocation{A53} to Average.
	\item Make a copy of the \fmtWorksheetName{Sales} sheet and rename it \fmtWorksheetName{Subtotals by Date}. Place this new sheet to the right of the \fmtWorksheetName{Sales by Product} sheet. Subtotal the sheet by Date (Oldest to Newest), summing the Profit column. Click the 2 Outline button to show just the subtotals by date and the grand total.
	\item Make one final copy of the \fmtWorksheetName{Sales} sheet and rename it \fmtWorksheetName{Subtotals by Type}. Place this new sheet to the right of the \fmtWorksheetName{Subtotals by Date} sheet. Subtotal the sheet by Sales Type, summing the Profit column.
	\item Add a second subtotal to the \fmtWorksheetName{Subtotals by Type} sheet that subtotals by Type and averages the Profit column. (Hint: uncheck \fmtRibbonButton{Replace Current Subtotals} in the Subtotal dialog box.) Notice that four outline buttons appear with the second subtotal. Figure out which Outline button to click to display both subtotals for Online and Retail and two Grand Totals.
