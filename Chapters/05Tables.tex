...
A temperature of $ 3.4 $ degrees was entered erroneously in the \textbf{Portland ME} sheet. It should have been $ 13.4 $. To fix this error, complete the following steps.

\begin{enumerate}
	\item Click on the \textbf{Portland ME} sheet.
	\item Press the \keystroke{Ctrl}+\keystroke{Home} keys together to go to the top of the sheet (\textsf{A1}).
...
4. Hold down the \keystroke{Ctrl} key and click on the Slicer buttons for Days 10 through 14. The table should now show the data from Days 10-15.
...
Subtotals and grand totals can be easily calculated for a column in a table. This is a powerful tool that to quickly display multiple levels of summary data within the table. This can
...
2. Point at the Weekly OR sheet tab at the bottom of the screen, hold the \keystroke{Ctrl} key down, and left-drag the sheet to the right until it is past all the existing sheets.
3. When a sheet icon with a + sign is visible, let go of the mouse button and then the \keystroke{Ctrl} key. A Weekly OR (2) sheet will appear.
4. Right-click on the new Sheet tab, select Rename, type Subtotal OR, and then press \keystroke{Enter}.
...
9. When a box pops up with a warning about converting the table, click Yes.
...
3. Navigate to the location where the data files for Chapter 5 are located and double-click on the Weather image file.
...
Sales.
2. Click on the Sales sheet. In I4 , enter a \formatfunc{Vlookup} function that will find the Product Price for
the Product in E4 in the table in the Product Table sheet and return it to I4. In the \formatfunc{Vlookup}
function, fill in the required parameters using Figure 5.32 below. Copy the \formatfunc{Vlookup} function
down column I.
...
\begin{figure}[H]
	...
	\caption{\formatfunc{Vlookup} window}
	\label{05:fig32}
\end{figure}
...
3. In J4 , enter a \formatfunc{Vlookup} function that will find the Product Cost for the Product in E4 in the
table in the Product Table sheet and return it to J4. This \formatfunc{Vlookup} function will be the same as
the \formatfunc{Vlookup} function in I4 EXCEPT THE COL\_INDEX\_NUM will be 3 instead of 4. Copy
