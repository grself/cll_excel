...
%fileclose CH8-Sparkline
%filesolution CH8-Sparkline
...
\begin{enumerate}[resume]
	...
	\item Click and drag \textit{Year} in the \textit{Field List} to the \fmtButton{Time} setting. Not much changes, but a timeline bar is added to the bottom of the map. The small triangle marker under the timeline can be moved left and right to display the population for any desired point of time.
...
\begin{itemize}
	...
	\item Format \fmtLoc{Column D} as percent's and increase the decimal places to two. When done, cell \fmtLoc{D5} should read $ 0.50 $\%.
...
A mortgage is one of the most common loans. A mortgage for a home is a huge investment and borrowers must think carefully about the size of the monthly payment compared to their disposable income. A two-variable data table is helpful in this type of decision.\footnote{See Chapter \ref{ch02:computations}, \nameref{ch02:computations}, page \pageref{ch02:computations} for more information about computing loans like mortgages.}
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item In cell \fmtLoc{B4}, enter \fmtTyping{=PMT(Interest\_Rate/12,Term\_in\_Years*12,Loan\_Amount)}. Given the information in \fmtLoc{B1:B3}, Excel calculates a payment of $ \$1,060.66 $ per month. This is a negative amount since the payment is going out of the borrower's savings.
	\end{itemize}

	\item Next, set up the data table area.
	\item In cell \fmtLoc{D4}, enter \fmtTyping{=Payment}. This will ``seed'' the \fmtButton{PMT} formula into the data table.
...
%filesave CH8-Data Table
	\item Save the \fmtWorksheet{CH8-Data Table} workbook.
%fileclose CH8-Data Table
%filesolution CH8-Data Table Solution
...
\begin{enumerate}
%fileopen CH8-Scenario
	\item Open workbook \fmtWorksheet{CH8-Scenario}.
%filesave CH8-Williams Budget
...
%fileclose CH8-Williams Budget
%filesolution CH8-Williams Budget Solution
...
%fileclose CH8-Goal Seek
%filesolution CH8-Goal Seek Solution
...
\begin{enumerate}[resume]
	...
	\item For \textit{By Changing Variable Cells} enter \fmtTyping{\$E\$6:\$E\$9}. This tells Excel to change the number of coaches allotted to minimize the cost for the number of passengers to carry.
...
%fileclose CH8-PeopleMover
%filesolution CH8-PeopleMover Solution
...
%fileclose PR8-Elections
%filesolution PR8-Elections Solution
...
%fileclose CH8-Cancer Map
%filesolution CH8-Cancer Map
