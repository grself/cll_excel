...
\begin{enumerate}
	...
	\item Click cell \fmtLoc{I6}. (\fmtLoc{I6} and \fmtLoc{I7} are merged together, but that does not matter.)
...
Take a look at the \textit{Notes} in the \textit{September} worksheet. It says that the rent was raised in September, so the \textit{Gym Membership} needs to be cancelled and $ \$0 $ should be entered for that amount in October, November, and December.
...
\begin{enumerate}[resume]
	\item Click the \fmtWorksheet{Expenses Summary} worksheet tab to activate it. The data and the pie chart should show the \textit{January} data since that is the only data in the twelve month worksheets. The worksheet should look like Figure \ref{06:fig14}.
	\item Save the file.
\end{enumerate}
...
\begin{enumerate}
	...
	\item To view all of the worksheets at one time, select \fmtButton{Print Entire Workbook} in the first box in the \textit{Settings} section. There should be $ 26 $ pages to scroll through in \textit{Print Preview}. At this point, clicking the \fmtButton{Print} button would print all of the worksheets rather than just the active sheet.
	...
	\item Group all of the worksheets together, including the \fmtWorksheet{Expenses Summary} worksheet through the \fmtWorksheet{December} worksheet.
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item Format \fmtLoc{F9:F19} as as percentages.
...
\begin{enumerate}[resume]
	...
	\item In cell \fmtLoc{D3}, enter the \textit{Player Name}: \fmtTyping{Zach Johnson}. 
...
\begin{enumerate}[resume]
	...
	\item In cell \fmtLoc{D3}, enter the \textit{Player Name}: \fmtTyping{Vito Lawrenz}. 
...
\begin{enumerate}[resume]
	...
	\item Enter a $ 3 $-D formula to \fmtTyping{COUNTA} in \fmtLoc{B9} on worksheets \fmtWorksheet{Player 1} through \fmtWorksheet{Player 5}. Note: this is easier if the \textit{Insert Function} dialog box is used.
	...
	\item Enter a $ 3 $-D formula to \fmtTyping{SUM} cells \fmtLoc{D9} on worksheets \fmtWorksheet{Player 1} through \fmtWorksheet{Player 5}. Note: this is easier if the \textit{Insert Function} dialog box is used.
