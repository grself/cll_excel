...
\begin{enumerate}
	...
	\item Highlight the range \fmtCellLocation{A3:A8} on the \fmtWorksheetName{Grade Distribution} worksheet. Column A shows the grade categories.
...
\begin{enumerate}
	...
	\item Select a \fmtRibbonButton{Callout: Line} shape at the bottom of the \fmtRibbonGroup{Shapes} menu of the \fmtRibbonGroup{Illustrations} group on the \fmtRibbonTab{Insert} tab.
	...
	\item The callout is initially filled with blue, but that does not offer enough contrast with the chart bar. The \fmtPopupBox{Format Shape} panel opens when the callout is added. Click the \fmtPopupBox{Shape Options} tab on the format panel and make these adjustments.
	...
	\item Click the callout box to highlight it and on the \fmtRibbonTab{Home} tab select Arial, $ 12 $pt, Bold font. Also, click the \fmtRibbonButton{Center} button to center the text in the box.
