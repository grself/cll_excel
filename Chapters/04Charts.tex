...
\begin{enumerate}
	...
	\item Select the range \fmtLoc{B4:C28}. (Notice that labels in the \fmtLoc{B4:C4} and all of the labels in \fmtLoc{B5:B28} are selected. Observe where they show up in the completed chart.)
...
\begin{enumerate}[resume]
	\item Change the chart title to \fmtTyping{Grade Distribution Comparison}. To add a chart title if one is not present, click \fmtButton{Design $ \Rightarrow $ Add Chart Element $ \Rightarrow $ Chart Title $ \Rightarrow $ Above Chart}.
	\item Save the workbook.
\end{enumerate}
...
\begin{enumerate}
	\item Click \fmtButton{Design $ \Rightarrow $ Data $ \Rightarrow $ Switch Row/Column}. This reverses the legend and current \textit{X-Y Axis} categories. (\fmtNewExcel{Excel 365} named the tab \textit{Chart Design}.)
\end{enumerate}
...
\begin{enumerate}[resume]
	...
	\item Click \fmtButton{Design $ \Rightarrow $ Add Chart Element $ \Rightarrow $ Data Table $ \Rightarrow $ With Legend Keys}.
	\item Change the Chart Title to \fmtTyping{Enrollment by Race}. If there is no chart title, click \fmtButton{Design $ \Rightarrow $ Add Chart Element $ \Rightarrow $ Chart Title $ \Rightarrow $ Above Chart}.
	\item Save the worksheet.
\end{enumerate}
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Click either the \textit{Home} tab or \textit{Design} tab of the ribbon.
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Click anywhere along the X Axis or Y Axis to activate it.
			\item Click \textit{Chart Tools $ \Rightarrow $ Current Selection $ \Rightarrow $ Format Selection}.
...
\begin{enumerate}
	...
	\item Click \fmtButton{Design $ \Rightarrow $ Add Chart Element $ \Rightarrow $ Axis Titles $ \Rightarrow $ Primary Vertical} (See Figure \ref{04:fig34}). (\fmtNewExcel{Excel 365} named this tab \textit{Chart Design}.)
\end{enumerate}
...
\begin{enumerate}
	\item Click \fmtButton{Design $ \Rightarrow $ Add Chart Element $ \Rightarrow $ Axis Titles $ \Rightarrow $ Primary Horizontal}.
	\item Using \fmtButton{Home $ \Rightarrow $ Font}, change the font of the axis title to Arial, Bold, Size $ 11 $. 
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item On the \textit{Design} tab select the \textit{Add Chart Element} button.
...
\begin{enumerate}
	\item Click on any of the the red bars representing the \textit{All Excel Classes} data series on the \textit{Grade Distribution Comparison} chart in the \fmtWorksheet{Grade Distribution} worksheet. Clicking one bar automatically activates all bars in the data series. If a bar is clicked a second time, only that one bar is activated.
	\item Right click and select \fmtButton{Format Data Series} to open up the \textit{Format Data Series} pane.
...
\begin{enumerate}
	...
	\item Click \fmtButton{Design $ \Rightarrow $ Add Chart Element $ \Rightarrow $ Data Labels $ \Rightarrow $ Outside End} (see Figure \ref{04:fig38}.)
	...
	\item Click on one of the data labels for the other data series. Using \fmtButton{Home $ \Rightarrow $ Font}, change the font to Arial, Bold, Size $ 9 $.
	\item Save the workbook.
\end{enumerate}
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Click anywhere on the chart to activate it.
			\item Click \textit{Design $ \Rightarrow $ Chart Layouts $ \Rightarrow $ Add Chart Element}.
...
\begin{enumerate}
	...
	\item Click \fmtButton{Design $ \Rightarrow $ Move Chart}. (\fmtNewExcel{Excel 365} named this tab \textit{Chart Design}.)
	...
	\item Click \fmtButton{Design $ \Rightarrow $ Add Chart Element $ \Rightarrow $ Axis Titles $ \Rightarrow $ Primary Vertical}.
	...
	\item Click \fmtButton{Design $ \Rightarrow $ Add Chart Element $ \Rightarrow $ Axis Titles $ \Rightarrow $ More Axis Title Options}.
	...
	\item Close the \textit{Format Chart Title} pane.
	\item Click \fmtButton{Design $ \Rightarrow $ Add Chart Elements $ \Rightarrow $ Lines $ \Rightarrow $ Series Lines}. This adds lines to the chart, connecting each data series between the three stacks (see Figure \ref{04:fig40}).
\end{enumerate}
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Click \textit{Design $ \Rightarrow $ Add Chart Elements $ \Rightarrow $ Lines $ \Rightarrow $ Series Lines}.
		\end{itemize}
	\end{sklbox}
\end{center}
...
\begin{enumerate}
	...
	\item The \textit{Copy Picture} diaglog box will open. Select \fmtButton{OK} --- Accepting the \textit{Copy Pictures} defaults:
...
\begin{enumerate}
	...
	\item Click \fmtButton{Design $ \Rightarrow $ Chart Style $ \Rightarrow $ Style $ 8 $}. This is the style with a dark brown background.
\end{enumerate}
...
\begin{enumerate}
	...
	\item If the chart has not changed, be sure that the chart is selected, click \fmtButton{Design $ \Rightarrow $ Chart Tools $ \Rightarrow $ Refresh Data}. The change made in the Excel workbook is now reflected on the PowerPoint slide.
...
\begin{enumerate}
	\item Close the print preview by clicking the arrow at the top left corner of the preview screen.
	\item Click \fmtButton{View $ \Rightarrow $ Workbook Views $ \Rightarrow $ Page Break Preview}. The screen should look similar to Figure \ref{04:fig53}. (Remember that the dotted blue lines indicate automatic page breaks.)
...
\begin{enumerate}
	...
	\item The line chart is create in the \fmtWorksheet{Minimum Wage} worksheet.
...
\begin{enumerate}[resume]
	...
	\item While the chart is still selected, click \fmtButton{Design $ \Rightarrow $ Chart Styles}. If necessary, press the More button to see the available styles.
...
\begin{enumerate}
	...
	\item Click \fmtButton{Design $ \Rightarrow $ Select Data}. This opens the \textit{Select Data Source} dialog box. The box at the top tells us the range that was selected, which looks fine. 
\end{enumerate}
...
\begin{enumerate}[resume]
	...
	\item The green line across the bottom of the chart represents the difference between men's and women's life expectancy. It is not very helpful as it is. Right click on the green line to open the context menu. Select \fmtButton{Format Data Series}. In the \textit{Format Data Series} pane, under the \fmtButton{Series Options} tab, select the radio button in front of \fmtButton{Secondary Axis}.
	\item Close the \textit{Format Data Series} pane.
	...
	\item Click \fmtButton{Design $ \Rightarrow $ Chart Styles} and select the first style.
