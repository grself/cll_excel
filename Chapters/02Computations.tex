...
\begin{enumerate}
	...
	\item Click cell \fmtLoc{F3} to activate it.
	\item Place the mouse pointer over the \fmtButton{Auto Fill Handle}.
...
\begin{enumerate}
	\item Click the \fmtWorksheet{Budget Detail} worksheet to open it if it is not already open.
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Click \fmtButton{Formulas $ \Rightarrow $ Formula Auditing $ \Rightarrow $ Show Formulas} to toggle the formula display on and off.
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Click \fmtButton{Formulas $ \Rightarrow $ Formula Auditing $ \Rightarrow $ Trace Dependents}.
			\item Use the arrow(s) to determine where the cell is referenced in formulas and functions.
			\item Click \fmtButton{Formulas $ \Rightarrow $ Formula Auditing $ \Rightarrow $ Remove Arrows} to remove the arrows from the worksheet.
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Click \fmtButton{Formulas $ \Rightarrow $ Formula Auditing $ \Rightarrow $ Trace Precedents}.
...
\begin{enumerate}
	...
	\item Highlight cells \fmtLoc{D12} and \fmtLoc{E12}.
...
\begin{center}
	\begin{infobox}{Integrity Check}
		...
		\\
		\\
		When using a statistical function on a range of cells in a worksheet, make sure the two cell locations are separated by a colon and not a comma. If two cell locations are separated by a comma, the function will produce an output but it will be applied to only two cell locations instead of the entire range of cells. For example, the \fmtButton{SUM} function shown in Figure \ref{02:fig13} is written with a comma and will add only the values in cells \fmtLoc{C3} and \fmtLoc{C11}, not the range \fmtLoc{C3:C11}.
	\end{infobox}
\end{center}
...
\begin{enumerate}
	...
	\item Click \fmtButton{Home $ \Rightarrow $ Clipboard $ \Rightarrow $ Copy}.
	\item Highlight the range \fmtLoc{B4:B11}.
...
\begin{enumerate}
	...
	\item Highlight the range \fmtLoc{D3:D11}.
...
\begin{enumerate}
	...
	\item Place the mouse pointer over the \fmtButton{Statistical} option from the drop-down list of options.
	\item Click the \fmtButton{AVERAGE} function name from the list of functions that appear in the menu (see Figure \ref{02:fig19}). This opens the \textit{Function Arguments} dialog box.
	\item Click the \fmtButton{Collapse Dialog} button for \textit{Number1} in the \textit{Function Arguments} dialog box (see Figure \ref{02:fig20}).
	\item Highlight the range \fmtLoc{D3:D11}.
...
\begin{enumerate}
	\item Highlight the range \fmtLoc{D13:D16} in the \fmtWorksheet{Budget Detail} worksheet.
	\item Click \fmtButton{Home $ \Rightarrow $ Clipboard $ \Rightarrow $ Copy}.
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Click \fmtButton{Home $ \Rightarrow $ Clipboard $ \Rightarrow $ Paste Down Arrow $ \Rightarrow $ Formulas}.
...
\begin{enumerate}
	\item Highlight the range \fmtLoc{A2:F11} in the \fmtWorksheet{Budget Detail} worksheet.
	...
	\item Select the \textit{LY Spend} option. Leave the Sort Order as \fmtButton{Smallest to Largest}
	\item Click the \fmtButton{OK} button at the bottom of the \textit{Sort} dialog box.
	\item Save the \fmtWorksheet{Ch2-Personal Budget} file.
\end{enumerate}
...
\begin{center}
	\begin{sklbox}{Skill Refresher}
		...
		\begin{itemize}
			...
			\item Select a column from the \fmtButton{Sort by} drop-down list in the \textit{Sort} dialog box.
			\item Select a sort order from the \fmtButton{Order} drop-down list in the \textit{Sort} dialog box.
			\item Click the \fmtButton{Add Level} button in the \textit{Sort} dialog box.
			\item Add as many levels as necessary.
			\item Click the \fmtButton{OK} button on the \textit{Sort} dialog box.
...
\begin{enumerate}
	...
	\item In the \textit{Search for a function:} input box at the top of the \textit{Insert Function} dialog box, type \fmtTyping{mortgage payments} (see Figure \ref{02:fig31}). 
	...
	\item Click the \fmtButton{PMT} option in the \textit{Select a function:} box in the lower half of the \textit{Insert Function} dialog box.
...
\begin{enumerate}[resume]
	...
	\item Click \fmtButton{OK} at the bottom of the \textit{Function Arguments} dialog box. The function will now be placed into the worksheet. Since there are no lump sum payments at the end of the loan, there is no need to define the Fv argument. Also, since the monthly mortgage payments will be made at the end of each month, there is no need to define the Type argument.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Highlight the range \fmtLoc{C3:C5}.
...
\begin{enumerate}
	\item \fmtLoc{A1:H1} --- Merge and Center; font is 14 point bold; fill color is Blue, Accent 1, Darker 25\%; font color is White
	\item \fmtLoc{A2:H2} --- Merge and Center; font is 12 point bold italic; fill color is Blue, Accent 1, Lighter 40\%; font color is Black
	\item \fmtLoc{A5:H5} --- font is bold; fill color is Blue, Accent 1, Lighter 40\%; font color is Black
	...
	\item \fmtLoc{A6:A14} (salespeople's names) and click \fmtButton{Home $ \Rightarrow $ Alignment $ \Rightarrow $ Increase Indent}. This will indent the text from the cell border.
\end{enumerate}
...
\begin{enumerate}
	\item With the \fmtWorksheet{CH2-Sales Data} file still open, click \fmtButton{File} to open the \textit{Backstage} view. 
	...
	\item Click \fmtButton{Page Setup $ \Rightarrow $ Margins} (see Figure \ref{02:fig43}).
	...
	\item Click the \fmtButton{Scaling} drop-down arrow in the Settings section (Figure \ref{02:fig43}).
...
\begin{enumerate}
	...
	\item Click cell \fmtLoc{C5} in the \fmtWorksheet{Annual Plan} worksheet.
	...
	\item Click cell \fmtLoc{F10} in the \fmtWorksheet{Leases} worksheet. Enter an Autosum function to calculate the total for the monthly lease payments. Make sure that blank rows (7 through 9) were included in the range for the \fmtButton{SUM} function so if other items are added later they will be included in the output of the \fmtButton{SUM} function.
	
	\item Highlight the range \fmtLoc{A2:F6} on the \fmtWorksheet{Leases} worksheet. The data in this range will be sorted, first by Interest Rate and then by Price. Click \fmtButton{Data $ \Rightarrow $ Sort \& Filter $ \Rightarrow $ Sort}. In the \textit{Sort} dialog box, select \textit{Interest Rate} in the \textit{Sort by} drop-down box. Select \textit{Largest to Smallest} for the sort order. Then, click \fmtButton{Add Level}. Select \textit{Price} in the \textit{Then by} drop-down box. Select \textit{Largest to Smallest} for the sort order. Click \fmtButton{OK}.
	
	\item Click cell \fmtLoc{B11} on the \fmtWorksheet{Annual Plan} worksheet. The monthly lease payments that are calculated in the \fmtWorksheet{Lease} worksheet will be displayed in this cell.
	...
	\item Click cell \fmtLoc{C12} on the \fmtWorksheet{Annual Plan} worksheet. Create a formula that calculates the annual lease payments. This should be the \textit{Monthly Lease Payments $ * 12 $}.
	
	\item Click cell \fmtLoc{C14} on the \fmtWorksheet{Annual Plan} worksheet. Create a formula that calculates the Total Lawn \& Equipment Expenses (\textit{Lawn \& Equipment Expenses per Cut * Total Lawn Cuts}).
	
	\item Click cell \fmtLoc{D16} on the \fmtWorksheet{Annual Plan} worksheet. Enter a \fmtButton{SUM} function that adds the Expenses for the business in column C. Make sure to add the Expenses only (not the Sales Plan information).
	
	\item Click cell \fmtLoc{D17} on the \fmtWorksheet{Annual Plan} worksheet. Enter a formula that calculates the annual profit (Operating Income) for the business. This should be the \textit{Total Sales $ – $ Total Expenses}.
	
	\item Format all cells that contain money amounts in the \fmtWorksheet{Annual Plan} worksheet for Accounting Number Format (\$) with no decimals.
	
	\item Click cell \fmtLoc{B10} on the \fmtWorksheet{Investments} worksheet. Enter a \fmtButton{COUNT} function that counts the number of investments that currently have a balance in \fmtLoc{Column B}. Make sure that the additional blank rows in \fmtLoc{Row 6} through \fmtLoc{Row 8} are included in the range for this function. The function output will automatically change if any new investments are added to the worksheet. It is important to note, however, that the \textit{Total} in cell \fmtLoc{B9} should not be included in the \textit{Count} range.
	
	\item Click cell \fmtLoc{D3} on the \fmtWorksheet{Investments} worksheet.
...
\begin{enumerate}
	...
	\item Enter a formula in cell \fmtLoc{E5} on the \fmtWorksheet{Occupancy} worksheet to calculate the Percent Occupied of the hotel (this statistic shows what percentage of the hotel is full or occupied). The formula should divide the Actual Occupancy by the Hotel Capacity. Then copy and paste the formula into the range \fmtLoc{E6:E16}. Use a paste method that does not remove the border at the bottom of cell \fmtLoc{E16}. Format the results in \fmtLoc{E5:E16} as percentages with two decimal places.
	...
	\item On the \fmtWorksheet{Statistics} worksheet, enter a function into cell \fmtLoc{B3} that shows the highest value (Max) in the range \fmtLoc{D5:D16} in the Actual Occupancy column on the \fmtWorksheet{Occupancy} worksheet.
	
	\item On the \fmtWorksheet{Statistics} worksheet, enter a function into cell \fmtLoc{B4} that shows the lowest value (Min) in the range \fmtLoc{D5:D16} in the Actual Occupancy column on the \fmtWorksheet{Occupancy} worksheet.
	
	\item On the \fmtWorksheet{Statistics} worksheet, enter a function into cell \fmtLoc{B5} that shows the average value in the range \fmtLoc{D5:D16} in the Actual Occupancy column on the \fmtWorksheet{Occupancy} worksheet.
