...
\begin{enumerate}
%fileopen CH7-Safeguard
%filesave CH7-Protected
...
%filesave CH7-WebData
	\item Save the workbook as \fmtWorksheet{CH7-WebData}.
%fileclose CH7-WebData
%filesolution CH7-WebData Solution
...
\begin{enumerate}[resume]
	\item To filter the data, click the down arrow to the right of the \textit{Column Labels} or \textit{Row Labels} headers on the pivot table (see Figure \ref{07:fig22}). Check only those regions or dates that should be included in the pivot table. For example, click the down arrow to the right of the \textit{Column Labels} and uncheck everything except \textit{East} then click \fmtButton{OK}. Notice how the pivot table is updated to include only the East region. Also notice that the dropdown includes options to sort the data in the pivot table.
\end{enumerate}
...
\begin{enumerate}[resume]	
	\item Click off the chart so it is no longer selected and then right-click on the chart title. Select \fmtButton{Edit Text} from the popup menu.
\end{enumerate}
...
\begin{enumerate}[resume]
	\item Right-click the \fmtWorksheet{Sales Pivot} worksheet and click \fmtWorksheet{Select All Sheets} to group all four worksheets. 
	\item Click \fmtButton{Page Layout $ \Rightarrow $ Page Setup $ \Rightarrow $ Launcher}.
	\item Click the \fmtButton{Header/Footer} tab.
	\item Click \fmtButton{Custom Header...}.
	\item Click in the right section to activate it and then click the \fmtButton{Insert Date} button.
	\item Click \fmtButton{OK}.
	\item Click \fmtButton{Custom Footer...}.
	\item Click in the center section to activate it and then click the \fmtButton{Insert Sheet Name} button.
	\item Click \fmtButton{OK}.
	...
	\item Click \fmtButton{File $ \Rightarrow $ Print} to enter the print preview. To view all the worksheets at one time, select \fmtButton{Print Entire Workbook} in the first box in the Settings section. There are $ 4 $ pages to scroll through in \textit{Print Preview}. Confirm that each worksheet is printing on one page with the date in the header and the worksheet name in the footer.
%filesave CH7-Sales Summary
	\item Save the \fmtWorksheet{CH7-Sales Summary} workbook.

%fileclose CH7-Sales Summary
%filesolution CH7-Sales Summary Solution	
...
%filesave PR7-Project
	\item Save the \fmtWorksheet{PR7-Project} workbook.
%fileclose PR7-Project
%filesolution PR7-Project Solution
