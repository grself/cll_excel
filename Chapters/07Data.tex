...
\begin{enumbox}
	\begin{enumerate}
		\item Open \fmtWorksheet{CH7-Safeguard}	and save the workbook as \fmtWorksheet{CH7-Protected}.
		...
		\item Check \fmtButton{In-cell drop-down}. This creates a drop-down list so the user can select a zip code from a list rather than enter it manually.
...
\begin{enumbox}
	\begin{enumerate}
		\item Copy \fmtLoc{E2} to \fmtLoc{E3:E11} so all those cells will also get the drop-down \textit{Zip Codes} validation list.
...
\begin{enumbox}
	\begin{enumerate}
		...
		\item This cell was initially set to \textit{Stop} on an error. Perhaps it would be acceptable for the user to finish the data entry even if it contained an error. Change the Style drop-down to \fmtButton{Warning}.
		\item Click \fmtButton{OK}.
		\item Copy \fmtLoc{F2} to \fmtLoc{F3:F11}.
		\item Click in \fmtLoc{F2} to activate that cell. Notice that the help message pops up. Enter some number in \fmtLoc{F2}. The entry works without error if that number is between $ 20 $ and $ 65 $. A warning message pops up if anything else is entered,  but the user can leave the incorrect age in place.
		...
		\item Enter these numbers into cells \fmtLoc{F2:F11}: \fmtTyping{$ 19 $, $ 50 $, $ 67 $, $ 40 $, $ 53 $, $ 35 $, $ 60 $, $ 23 $, $ 65 $, $ 21 $}. A warning should pop up for \fmtLoc{F2} and \fmtLoc{F4} since those entries are out of the acceptable range, but click \fmtButton{Yes} on the warning pop-up box, so the data is entered.
...
\begin{enumbox}
	\begin{enumerate}
		...
		\item Save the \fmtWorksheet{CH7-Protected} workbook.
		\item Compare the workbook with the self-check answer key (\fmtWorksheet{CH7-Protected Solution}) and then close and submit the \fmtWorksheet{CH7-Protected} workbook as directed by the instructor.
	\end{enumerate}
\end{enumbox}
...
\begin{enumbox}
	\begin{enumerate}
		...
		\item Select the \fmtButton{Basic} option and enter \fmtTyping{https://bit.ly/AdExcelC} in the \textit{From Web} pop-up, as shown in Figure \ref{07:fig02a}.
...
\begin{enumbox}
	\begin{enumerate}
		\item \fmtOldExcel{Excel 2016} Click \fmtButton{Data $ \Rightarrow $ Get \& Transform $ \Rightarrow $ New Query $ \Rightarrow $ From Other Sources $ \Rightarrow $ From Web}
		\item \fmtNewExcel{Excel 365} Click \fmtButton{Data $ \Rightarrow $ Get \& Transform Data $ \Rightarrow $ Get Data $ \Rightarrow $ From Other Sources $ \Rightarrow $ From Web}
		\item Enter \fmtTyping{https://bit.ly/AdExcelB} in the \textit{From Web} pop-up, as shown in Figure \ref{07:fig02a}, and click \fmtButton{OK}.
		...
		\item Save the workbook as \fmtWorksheet{CH7-WebData}.
		\item Compare the workbook with the self-check answer key (\fmtWorksheet{CH7-WebData Solution}) and then close and submit the \fmtWorksheet{CH7-WebData} workbook as directed by the instructor.
	\end{enumerate}
\end{enumbox}
...
\subsection{Introduction to Pivot Tables}

\begin{enumbox}
	\begin{enumerate}
		\item Open the \fmtWorksheet{CH7-Sales Summary} workbook that was created in section \ref{07:ImportFromDataFile} if it is not already opened.
		...
		\item In the \textit{Create PivotTable} pop-up, as seen in Figure \ref{07:fig12}, ensure that Excel automatically selected either the name of the data table, \textit{CH7\_Data}, or \$A\$1:\$G\$44, which is all the data on the worksheet. Also, ensure \fmtButton{New Worksheet} is selected as the location of the new pivot table. 
		\item Click \fmtButton{OK}.
		...
		\begin{enumerate}
			...
			\item Select \fmtButton{Average} in the pop-up, as illustrated in Figure \ref{07:fig17}.
			\item Click \fmtButton{OK}.
		\end{enumerate}
		...
		\begin{enumerate}
			...
			\item Select \fmtButton{Value Field Settings}.
			\item Click the \fmtButton{Number Format} button at the bottom left corner of the \textit{Value Field Settings} pop-up, as illustrated in Figure \ref{07:fig18}.
...
\subsection{Using Calendar Data in a Pivot Table}

\begin{enumbox}
	\begin{enumerate}
		\item Create a new pivot table by clicking cell \fmtLoc{A1} in the \fmtWorksheet{Sales Data} worksheet.
		\item Click \fmtButton{Insert $ \Rightarrow $ Tables $ \Rightarrow $ PivotTable}.
		\item In the \textit{Create PivotTable} pop-up, ensure that Excel automatically selects the data table name, \textit{CH7\_Data}, or \$A\$1:\$G\$44, which is all the data on the worksheet. Also, ensure \fmtButton{New Worksheet} is selected as the location of the new pivot table. 
		\item Click \fmtButton{OK}.
		...
		\item To filter the data, click the down arrow to the right of the \textit{Column Labels} or \textit{Row Labels} headers on the pivot table (see Figure \ref{07:fig22}). Check only those regions or dates that should be included in the pivot table. For example, click the down arrow to the right of the \textit{Column Labels}, uncheck everything except \textit{East}, and then click \fmtButton{OK}. Notice how the pivot table is updated to include only the East region. Also, notice that the drop-down includes options to sort the data in the pivot table.
		...
		\item All regions should be included for the rest of this exercise, so click the down arrow to the right of the \textit{Column Labels} and check \textit{Select All}, then click \fmtButton{OK}.
		\item While managers usually like having dates grouped by month/quarter/year, that can be easily modified. Click cell \fmtLoc{A5} in the pivot table, so only one cell is selected. Click \fmtButton{Analyze $ \Rightarrow $ Group $ \Rightarrow $ Group Field}.
		...
		\item In the \textit{Grouping} pop-up box, the start/end dates for the data can be specified to limit the data shown in the pivot table. For this exercise, leave the dates at their default (two entire years), click \textit{months} so it is no longer selected, then click \fmtButton{OK}. Figure \ref{07:fig24} illustrates the \textit{Grouping} pop-up box after the \fmtButton{Months} item is inactivated. This process removes the \textit{Months} drill-down capability and only shows years and quarters. By using this option, details can be removed from the pivot table so trends may become more visible.
...
\begin{enumbox}
	\begin{enumerate}
		...
		\item In the \textit{Create PivotChart} pop-up, ensure that Excel automatically selected either the data table by name, \textit{CH7\_Data}, or \$A\$1:\$G\$44, which is all the data on the worksheet. Also, ensure \fmtButton{New Worksheet} is selected as the location of the new pivot chart.
		\item Click \fmtButton{OK}.
		...
		\item In the \textit{Field List}, click the checkboxes for \fmtButton{Rep} and \fmtButton{Total}. Excel creates a bar chart showing each Representative's total sales, as in Figure \ref{07:fig28}.
		...
		\item The bar chart automatically updates to display the total sales for each item in the inventory. However, this chart would be better if it were a pie chart.
		\item Click the down arrow for the \fmtButton{Sum of Total} item in the \textit{Values} area of the pivot table.
		\item Select \fmtButton{Value Field Settings} in the pop-up options menu.
		\item Click the \fmtButton{Show Values As} tab in the \textit{Value Field Settings} pop-up box.
		...
		\item Click off the chart so it is no longer selected and then right-click on the chart title. Select \fmtButton{Edit Text} from the pop-up menu.
		...
		\item Enter \fmtTyping{Sales by Item} as the chart title and then click off the chart so it is no longer selected.
...
\begin{enumbox}
	\begin{enumerate}
		...
		\item Click \fmtButton{File $ \Rightarrow $ Print} to enter the print preview. To view all the worksheets at one time, select \fmtButton{Print Entire Workbook} in the first box in the Settings section. There are $ 6 $ pages to scroll through in \textit{Print Preview}. 
		...
		\item Activate the \fmtWorksheet{Sales By Item} worksheet by clicking that tab. Notice that part of the chart is printed on a second page, as indicated by a dotted line running down the edge of column J. 
		...
		\item Right-click the \fmtWorksheet{Sales Pivot} worksheet and \fmtWorksheet{Select All Sheets} to group all four worksheets. 
		\item Click \fmtButton{Page Layout $ \Rightarrow $ Page Setup $ \Rightarrow $ Launcher}.
		\item Click the \fmtButton{Header/Footer} tab.
		\item Click \fmtButton{Custom Header$\ldots$}.
		\item Click the right section to activate it, and then click the \fmtButton{Insert Date} button.
		\item Click \fmtButton{OK}.
		\item Click \fmtButton{Custom Footer$\ldots$}.
		\item Click the center section to activate it, and then click the \fmtButton{Insert Sheet Name} button.
		\item Click \fmtButton{OK}.
		\item Click \fmtButton{OK} to close the \textit{Page Setup} dialog box.
		\item Right-click the \fmtWorksheet{Sales Pivot} worksheet and select \fmtButton{Ungroup Sheets}.
		\item Click \fmtButton{File $ \Rightarrow $ Print} to enter the print preview. To view all the worksheets at one time, select \fmtButton{Print Entire Workbook} in the first box in the Settings section. There are four pages to scroll through in \textit{Print Preview}. Confirm that each worksheet prints on one page. The date should be in the header and the worksheet name in the footer.
		\item Save the \fmtWorksheet{CH7-Sales Summary} workbook.
	
	\item Compare the workbook with the self-check answer key (\fmtWorksheet{CH7-Sales Summary Solution}) and then close and submit the \fmtWorksheet{CH7-Sales Summary} workbook as directed by the instructor.
...
\begin{enumbox}
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\item In the pop-up menu, select \fmtButton{Sort $ \Rightarrow $ Sort Largest to Smallest} to sort the total billed from the largest to the smallest amount.
		...
		\item Save the \fmtWorksheet{PR7-Project} workbook.
		\item Compare the workbook with the self-check answer key (\fmtWorksheet{PR7-Project Solution}) and then close and submit the \fmtWorksheet{PR7-Project} workbook as directed by the instructor.
...
\begin{enumbox}
	\begin{enumerate}
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=\maxwidth{.60\linewidth}]{gfx/ch07_fig58}
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=\maxwidth{.50\linewidth}]{gfx/ch07_fig59}
			\caption{Average Age}
			\label{07:fig59}
		\end{figure}
		...
		\begin{enumerate}
			...
			\item The pivot chart and worksheet should be named \fmtWorksheet{Count By Role} and moved to the right of the \fmtWorksheet{Year Hired} worksheet.
		...
		\item Submit the \fmtWorksheet{SC7-Employees} workbook as directed by the instructor.
	\end{enumerate}
\end{enumbox}